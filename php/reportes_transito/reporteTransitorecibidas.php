...
$registro = "SELECT DATE_FORMAT(tr.fecha, '%d/%m/%Y') As 'fecha', CONCAT(p.nombre,' ',p.apellido) AS 'nombre', tr.expediente As 'expediente', p.identidad AS 'identidad', tr.edad As 'edad', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', (CASE WHEN tr.paciente = 'n' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN tr.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente', d.nombre As 'departamento', m.nombre AS 'municipio', s.nombre AS 'recibidade', pc.nombre AS 'recibidade_unidad', tr.observacion AS 'observacion' , CONCAT(c.nombre,' ',c.apellido) AS 'medico'
