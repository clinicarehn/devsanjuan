...
$registro = "SELECT DATE_FORMAT(te.fecha, '%d/%m/%Y') AS 'fecha', CONCAT(p.nombre,' ',p.apellido) AS 'nombre', te.expediente AS 'expediente', p.identidad AS 'identidad', te.edad AS 'edad', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', (CASE WHEN te.paciente = 'n' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN te.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente', d.nombre As 'departamento', m.nombre AS 'municipio', s.nombre AS 'enviadaa', pc.nombre AS 'enviadaa_unidad', te.observacion AS 'observacion', CONCAT(c.nombre,' ',c.apellido) AS 'medico'
