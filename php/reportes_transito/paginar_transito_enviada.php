...
$query = "SELECT te.transito_id AS 'transito_id', DATE_FORMAT(te.fecha, '%d/%m/%Y') AS 'fecha', CONCAT(p.nombre,' ',p.apellido) AS 'nombre', te.expediente AS 'expediente', p.identidad AS 'identidad', te.edad AS 'edad', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', (CASE WHEN te.paciente = 'n' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN te.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente', d.nombre As 'departamento', m.nombre AS 'municipio', s.nombre AS 'enviadaa', pc.nombre AS 'enviadaa_unidad', te.observacion AS 'observacion', CONCAT(c.nombre,' ',c.apellido) AS 'medico'
   ...
   ORDER BY te.fecha, te.expediente ASC";
...
$registro = "SELECT te.transito_id AS 'transito_id', DATE_FORMAT(te.fecha, '%d/%m/%Y') AS 'fecha', CONCAT(p.nombre,' ',p.apellido) AS 'nombre', te.expediente AS 'expediente', p.identidad AS 'identidad', te.edad AS 'edad', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', (CASE WHEN te.paciente = 'n' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN te.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente', d.nombre As 'departamento', m.nombre AS 'municipio', s.nombre AS 'enviadaa', pc.nombre AS 'enviadaa_unidad', te.observacion AS 'observacion', CONCAT(c.nombre,' ',c.apellido) AS 'medico'
   FROm transito_enviada AS te
   INNER JOIN pacientes AS p
   ON te.expediente = p.expediente
   INNER JOIN departamentos AS d
   ...
   ON te.colaborador_id = c.colaborador_id		   
   WHERE te.fecha BETWEEN '$desde' AND '$hasta' AND te.servicio_id = '$servicio' AND (p.expediente LIKE '%$dato%' OR CONCAT(p.nombre,' ',p.apellido) LIKE '%$dato%' OR p.apellido LIKE '$dato%' OR p.identidad LIKE '$dato%')
   ORDER BY te.fecha, te.expediente
...
while($registro2 = $result->fetch_assoc()){	
	$tabla = $tabla.'<tr>
	   ...
	   <td>
		   <a title = "Eliminar Registro" href="javascript:modal_eliminarTransitoEnviada('.$registro2['transito_id'].','.$registro2['expediente'].');void(0);" class="fas fa-trash fa-lg" style="text-decoration:none;"></a>
	   </td>		   
	</tr>';	        
}
