...
function getAgendatime($consultarJornadaJornada_id, $servicio, $consultarJornadaServicio_id, $consultar_colaborador_puesto_id, $expediente, $hora_h, $consulta_nuevos_devuelto, $consultarJornadaNuevos, $consultaJornadaTotal, $consulta_subsiguientes_devuelto){
   ...
   $hora = '';
	...
	if($consultarJornadaJornada_id != "" && $servicio == $consultarJornadaServicio_id){//########INICIO EVALUAMOS SI EL PROFESIONAL TIENEN UNA JORNADA ASIGNADA########
					...
					if ($hora_h >= date('H:i',strtotime('07:20')) && $hora_h < date('H:i',strtotime('9:20'))){//INICIO USUARIOS NUEVOS
						...
						if($consulta_nuevos_devuelto > $consultarJornadaNuevos){
						   ...
						   if ( $expediente == ""){//EVALUA SI ES UN USUARIO NUEVO PARA EL CONTROL DE HORAS
							  if ($hora_h >= date('H:i',strtotime('09:20')) && $hora_h < date('H:i',strtotime('18:40'))){
					 ...
					 else{//INICIO USUARIOS SUBSIGUIENTES
				...
				if($consultarJornadaJornada_id == 1){//####INICIO JORNADA MATUTINA
					...
					if ($hora_h >= date('H:i',strtotime('07:20')) && $hora_h < date('H:i',strtotime('09:20'))){//INICIO NUEVOS
						...
						if($consulta_nuevos_devuelto > $consultarJornadaNuevos){
						   ...
						   if ( $expediente == ""){//EVALUA SI ES UN USUARIO NUEVO PARA EL CONTROL DE HORAS
							  if ($hora_h >= date('H:i',strtotime('9:20')) && $hora_h < date('H:i',strtotime('17:20'))){
								 $hora = 'NulaN';//ESTE ES UN USUARIO NUEVO, NO ES SUBSIGUIENTE
							  }
						   }else{//EVALUA SI ES UN USUARIO SUBSIGUIENTE PARA EL CONTROL DE HORAS
							  if ($hora_h >= date('H:i',strtotime('07:20')) && $hora_h < date('H:i',strtotime('09:20'))){
						 ...
						 if ( $expediente == ""){//EVALUA SI ES UN USUARIO NUEVO PARA EL CONTROL DE HORAS
							 if ($hora_h >= date('H:i',strtotime('09:20')) && $hora_h < date('H:i',strtotime('17:20'))){
								$hora = 'NulaN';//ESTE ES UN USUARIO NUEVO, NO ES UN SUBSIGUIENTE
							 }
						 }else{///EVALUA SI ES UN USUARIO SUBSIGUIENTE PARA EL CONTROL DE HORAS
							 if ($hora_h >= date('H:i',strtotime('07:20')) && $hora_h < date('H:i',strtotime('09:20'))){
					...
					if ($hora_h >= date('H:i',strtotime('13:20')) && $hora_h < date('H:i',strtotime('23:20'))){//INICIO NUEVOS
						...
						if($consulta_nuevos_devuelto > $consultarJornadaNuevos){
						   ...
						   if ($hora_h >= date('H:i',strtotime('13:20')) && $hora_h < date('H:i',strtotime('14:00'))){
							  $hora = "13:00"; //HORA PARA USUARIOS NUEVOS POR LA MAÑANA
						   }else if ($hora_h >= date('H:i',strtotime('14:00')) && $hora_h < date('H:i',strtotime('14:40'))){
							  $hora = "13:00"; //HORA PARA USUARIOS NUEVOS POR LA MAÑANA
						   }else if ($hora_h >= date('H:i',strtotime('14:40')) && $hora_h < date('H:i',strtotime('17:20'))){
							  $hora = "13:00"; //HORA PARA USUARIOS NUEVOS POR LA MAÑANA
						   }else if($hora_h == date('H:i',strtotime('00:00')) ){
							 $hora = 'NulaSError';
						   }else{
						   ...
						   if ( $expediente == ""){//EVALUA SI ES UN USUARIO NUEVO PARA EL CONTROL DE HORAS
							  ...
							  if ($hora_h >= date('H:i',strtotime('13:20')) && $hora_h < date('H:i',strtotime('17:20'))){
								 $hora = 'NulaS';//ESTE ES UN USUARIO SUBSIGUIENTE, NO ES NUEVO
							  }
						   }
						 }
				...
				if($consultarJornadaJornada_id == 1){//####INICIO JORNADA MATUTINA
					...
					if ($expediente == ""){
						...
						if($consulta_nuevos_devuelto > $consultarJornadaNuevos){
							...
							/*
								if ($hora_h >= date('H:i',strtotime('12:00')) && $hora_h < date('H:i',strtotime('12:40'))){
		...
		}//#####FIN SERVICIO UNIDA DE NIÑOS Y ADOLESCENTES#####
		else{//INICIO PARA TODOS LOS OTROS SERVICIOS

			if ($expediente == ""){//INICIO PARA EL INGRESO USUARIOS NUEVOS
				$colores = "#008000"; //VERDE USUARIOS NUEVOS
				if($consulta_nuevos_devuelto > $consultarJornadaNuevos){
					$hora = "NuevosExcede";
				}else{
					if ($hora_h >= date('H:i',strtotime('12:00')) && $hora_h < date('H:i',strtotime('12:40'))){
						$hora = "Nula"; //EVLAUA LA HORA DE ALMUERZO Y NO PERMITE AGENDAR USUARIOS EN ELLA
					}else if ($hora_h >= date('H:i',strtotime('07:20')) && $hora_h < date('H:i',strtotime('12:00'))){
						$hora = $hora_h;
					}else if ($hora_h >= date('H:i',strtotime('12:40')) && $hora_h < date('H:i',strtotime('16:00'))){
					   $hora = $hora_h;
					}else{
					   $hora = "NulaP"; //HORA NO PERMITIDA PARA AGENDAR
					}
				...
				if($consulta_subsiguientes_devuelto > $limite){
					$hora = "SubsiguienteExcede";
				}else if ($hora_h >= date('H:i',strtotime('12:00')) && $hora_h < date('H:i',strtotime('12:40'))){
					 $hora = "Nula"; //EVLAUA LA HORA DE ALMUERZO Y NO PERMITE AGENDAR USUARIOS EN ELLA
				}else if ($hora_h >= date('H:i',strtotime('07:20')) && $hora_h < date('H:i',strtotime('12:00'))){
					 $hora = $hora_h;
				}else if ($hora_h >= date('H:i',strtotime('12:40')) && $hora_h < date('H:i',strtotime('16:00'))){
		...
		}//FIN PARA TODOS LOS OTROS SERVICIOS
	}//########FIN EVALUAMOS SI EL PROFESIONAL TIENEN UNA JORNADA ASIGNADA########
					...
					if ($hora_h >= date('H:i',strtotime('07:20')) && $hora_h < date('H:i',strtotime('09:20'))){//INICIO NUEVOS
					   ...
					   if($consulta_nuevos_devuelto > $consultarJornadaNuevos){
						...
						if($consulta_subsiguientes_devuelto > $limite){
						  ...
						  $hora = "09:40"; //HORA PARA USUARIOS SUBSIGUIENTES POR LA TARDE
						}else if($hora_h >= date('H:i',strtotime('20:40')) && $hora_h < date('H:i',strtotime('21:20'))){//7MO USUARIO SUBSIGUIENTE
						  $hora = "10:20"; //HORA PARA USUARIOS SUBSIGUIENTES POR LA TARDE
						}else if($hora_h >= date('H:i',strtotime('21:20')) && $hora_h < date('H:i',strtotime('22:00'))){//8AVO USUARIO SUBSIGUIENTE
						  $hora = "11:00"; //HORA PARA USUARIOS SUBSIGUIENTES POR LA TARDE
						}else if($hora_h >= date('H:i',strtotime('22:20')) && $hora_h < date('H:i',strtotime('22:40'))){//9NO USUARIO SUBSIGUIENTE
						  $hora = "11:40"; //HORA PARA USUARIOS SUBSIGUIENTES POR LA TARDE
						}else if($hora_h >= date('H:i',strtotime('22:40')) && $hora_h < date('H:i',strtotime('23:20'))){//10MO USUARIO SUBSIGUIENTE
						  $hora = "12:20"; //HORA PARA USUARIOS SUBSIGUIENTES POR LA TARDE
						}else if($hora_h == date('H:i',strtotime('00:00')) ){
						  $hora = 'NulaSError';
						}else{
							$hora = "NulaP";
...
function getDisponibilidadDiponibilidadHorarioColaborador($colaborador_id, $fecha_cita){
	...
	$query = "SELECT agenda_id
		FROM agenda
		WHERE colaborador_id = '$colaborador_id' AND fecha_cita = '$fecha_cita' AND status = 0";
...
function sendMail($servidor, $puerto, $contraseña, $CharSet, $SMTPSecure, $de, $para, $from, $asunto, $mensaje){
