...
$registro = "SELECT DATE_FORMAT(d.fecha, '%d/%m/%Y') AS 'fecha_depuracion', CONCAT(p.nombre,' ',p.apellido) AS 'nombre', d.expediente As 'expediente', p.identidad AS 'identidad', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', d.diagnostico, DATE_FORMAT(d.fecha_ultima, '%d/%m/%Y') AS 'fecha_ultima', d.status AS 'status', CONCAT(c.nombre,' ',c.apellido) AS 'usuario', CONCAT(c1.nombre,' ',c1.apellido) AS 'colaborador', CONCAT(c2.nombre,' ',c2.apellido) AS 'colaborador1', s.nombre AS 'servicio'
       FROM depurados AS d
...
$objPHPExcel->getProperties()->setTitle("Reporte de Usuarios Depurados"); //titulo
...
$objPHPExcel->getActiveSheet()->setTitle("Reporte Depurados"); //establecer titulo de hoja
...
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Reporte de Usuarios Depurados");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:N$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:N$fila");
...
$objPHPExcel->getActiveSheet()->SetCellValue("B$fila", 'Fecha de Depuración');
...
$objPHPExcel->getActiveSheet()->SetCellValue("H$fila", 'Dx Principal');
$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(25);
$objPHPExcel->getActiveSheet()->mergeCells("H4:H5"); //unir celdas
$objPHPExcel->getActiveSheet()->SetCellValue("I$fila", 'Fecha Última Consulta');
...
$objPHPExcel->getActiveSheet()->SetCellValue("N$fila", 'Condición');
...
if($result->num_rows>0){
	while($registro2 = $result->fetch_assoc()){
       ...
       $objPHPExcel->getActiveSheet()->SetCellValue("B$fila", $registro2['fecha_depuracion']);
	   ...
	   $objPHPExcel->getActiveSheet()->SetCellValue("H$fila", $registro2['diagnostico']);
	   $objPHPExcel->getActiveSheet()->SetCellValue("I$fila", $registro2['fecha_ultima']);	   
       $objPHPExcel->getActiveSheet()->SetCellValue("J$fila", $registro2['colaborador']);
	   $objPHPExcel->getActiveSheet()->SetCellValue("K$fila", $registro2['colaborador1']);
	   ...
	   $objPHPExcel->getActiveSheet()->SetCellValue("N$fila", $status);	
...
// nombre del archivo
header('Content-Disposition: attachment; filename="Reporte de Usuarios Depurados '.$mes.'_'.$año.'.xls"');
