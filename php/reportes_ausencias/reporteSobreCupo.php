...
if($servicio != "" && $unidad == "" && $profesional == "" && $reporte != ""){
  ...
  $where = "WHERE so.fecha BETWEEN '$desde' AND '$hasta' AND so.servicio_id = '$servicio' AND c.puesto_id = '$unidad'";				
}else{
...
$objPHPExcel->getActiveSheet()->setTitle("Reporte Sobre Cupo"); //establecer titulo de hoja
...
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Registro de Usuarios Sobre Cupo $servicio_name");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:L$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:L$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="Reporte de Usuarios Sobre Cupo '.$servicio_name.' '.$mes.'_'.$año.'.xls"');
