...
if($unidad == "" && $colaborador_usuario == ""){
	$where = "WHERE aus.fecha BETWEEN '$desde' AND '$hasta' AND aus.servicio_id = '$servicio'";
	...
	$where = "WHERE aus.fecha BETWEEN '$desde' AND '$hasta' AND aus.servicio_id = '$servicio' AND c.puesto_id = '$unidad'";
}else{
    //OBTENER NOMBRE UNIDAD
    $consulta_unidad = "SELECT nombre 
	     FROM puesto_colaboradores 
	...
	$where = "WHERE aus.fecha BETWEEN '$desde' AND '$hasta' AND aus.servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND aus.usuario = '$colaborador_usuario'";	
}

//EJECUTAMOS LA CONSULTA DE BUSQUEDA
//REGISTROS
$registro = "SELECT CONCAT(c.nombre,' ',c.apellido) AS 'colaborador', (CASE WHEN aus.paciente = 'N' THEN 'Nuevo' ELSE 'Subsiguiente' END) AS 'Paciente',
COUNT(CASE WHEN DAY(aus.fecha) = 1 THEN aus.paciente END) AS '1',  
COUNT(CASE WHEN DAY(aus.fecha) = 2 THEN aus.paciente END) AS '2',
COUNT(CASE WHEN DAY(aus.fecha) = 3 THEN aus.paciente END) AS '3',
COUNT(CASE WHEN DAY(aus.fecha) = 4 THEN aus.paciente END) AS '4',
COUNT(CASE WHEN DAY(aus.fecha) = 5 THEN aus.paciente END) AS '5',
COUNT(CASE WHEN DAY(aus.fecha) = 6 THEN aus.paciente END) AS '6',
COUNT(CASE WHEN DAY(aus.fecha) = 7 THEN aus.paciente END) AS '7',
COUNT(CASE WHEN DAY(aus.fecha) = 8 THEN aus.paciente END) AS '8',
COUNT(CASE WHEN DAY(aus.fecha) = 9 THEN aus.paciente END) AS '9',
COUNT(CASE WHEN DAY(aus.fecha) = 10 THEN aus.paciente END) AS '10',
COUNT(CASE WHEN DAY(aus.fecha) = 11 THEN aus.paciente END) AS '11',  
COUNT(CASE WHEN DAY(aus.fecha) = 12 THEN aus.paciente END) AS '12',
COUNT(CASE WHEN DAY(aus.fecha) = 13 THEN aus.paciente END) AS '13',
COUNT(CASE WHEN DAY(aus.fecha) = 14 THEN aus.paciente END) AS '14',
COUNT(CASE WHEN DAY(aus.fecha) = 15 THEN aus.paciente END) AS '15',
COUNT(CASE WHEN DAY(aus.fecha) = 16 THEN aus.paciente END) AS '16',
COUNT(CASE WHEN DAY(aus.fecha) = 17 THEN aus.paciente END) AS '17',
COUNT(CASE WHEN DAY(aus.fecha) = 18 THEN aus.paciente END) AS '18',
COUNT(CASE WHEN DAY(aus.fecha) = 19 THEN aus.paciente END) AS '19',
COUNT(CASE WHEN DAY(aus.fecha) = 20 THEN aus.paciente END) AS '20',
COUNT(CASE WHEN DAY(aus.fecha) = 21 THEN aus.paciente END) AS '21',  
COUNT(CASE WHEN DAY(aus.fecha) = 22 THEN aus.paciente END) AS '22',
COUNT(CASE WHEN DAY(aus.fecha) = 23 THEN aus.paciente END) AS '23',
COUNT(CASE WHEN DAY(aus.fecha) = 24 THEN aus.paciente END) AS '24',
COUNT(CASE WHEN DAY(aus.fecha) = 25 THEN aus.paciente END) AS '25',
COUNT(CASE WHEN DAY(aus.fecha) = 26 THEN aus.paciente END) AS '26',
COUNT(CASE WHEN DAY(aus.fecha) = 27 THEN aus.paciente END) AS '27',
COUNT(CASE WHEN DAY(aus.fecha) = 28 THEN aus.paciente END) AS '28',
COUNT(CASE WHEN DAY(aus.fecha) = 29 THEN aus.paciente END) AS '29',
COUNT(CASE WHEN DAY(aus.fecha) = 30 THEN aus.paciente END) AS '30',
COUNT(CASE WHEN DAY(aus.fecha) = 31 THEN aus.paciente END) AS '31',
COUNT(aus.paciente) AS 'Total'
FROM ausencias AS aus
INNER JOIN colaboradores AS c
ON aus.colaborador_id = c.colaborador_id
".$where."
GROUP BY c.colaborador_id, aus.paciente";
...
$fila=2;
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Reporte de Ausencias. ".$servicio_name.' '.$unidad_name);
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:AH$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:AH$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="REPORTE DIARIO DE AUSENCIAS'.strtoupper($servicio_name).' '.strtoupper($unidad_name).' '.strtoupper($mes).'_'.$año.'.xls"');
