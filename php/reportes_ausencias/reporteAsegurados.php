...
if($servicio != "" && $unidad == "" && $profesional == "" && $reporte != ""){
  $where = "WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id = '$servicio' AND a.asegurado = 1";		
}else if ($servicio != "" && $unidad != "" && $profesional == "" && $reporte != ""){
  $where = "WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id = '$servicio' AND a.asegurado = 1 AND c.puesto_id = '$unidad'";				
}else{
  $where = "WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id = '$servicio' AND a.asegurado = 1 AND c.puesto_id = '$unidad' AND c.colaborador_id = '$profesional' ";
}
	
$registro = "SELECT DISTINCT DATE_FORMAT(CAST(a.fecha_registro AS DATE ), '%d/%m/%Y %H:%m:%s') AS 'fecha_registro', DATE_FORMAT(a.fecha, '%d/%m/%Y') AS 'fecha', p.identidad AS 'identidad', p.expediente AS 'expediente', CONCAT(p.apellido,' ',p.nombre) AS  'nombre', a.expediente AS 'expediente', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', s.nombre AS 'servicio', CONCAT(c.nombre,' ',c.apellido) AS 'medico', (CASE WHEN a.paciente = 'N' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN a.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente'
       FROM asegurado AS a
...
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Registro de Usuarios Asegurados (IHSS) $servicio_name");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:L$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:L$fila");
...
if($result->num_rows>0){
	while($registro2 = $result->fetch_assoc()){
	   ...
	   if($registro2['fecha_registro'] == '00/00/0000 00:00:00'){
		   $fecha_registro_ = $registro2['fecha'];
	   }else{
		   $fecha_registro_ = $registro2['fecha_registro']; 
	   }
	   $objPHPExcel->getActiveSheet()->SetCellValue("A$fila", $valor);
       $objPHPExcel->getActiveSheet()->SetCellValue("B$fila", $fecha_registro_);  
