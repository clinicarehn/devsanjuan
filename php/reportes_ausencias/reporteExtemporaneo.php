...
if($servicio != "" && $unidad == "" && $profesional == "" && $colaborador_usuario == ""){
    $where = "WHERE ex.fecha BETWEEN '$desde' AND '$hasta' AND ex.servicio_id = '$servicio'";		
}else if ($servicio != "" && $unidad != "" && $profesional == "" && $colaborador_usuario == ""){
    $where = "WHERE ex.fecha BETWEEN '$desde' AND '$hasta' AND pc.puesto_id = '$unidad ' AND ex.servicio_id = '$servicio'";				
}else if ($servicio != "" && $unidad != "" && $profesional != "" && $colaborador_usuario == ""){
    $where = "WHERE ex.fecha BETWEEN '$desde' AND '$hasta' AND pc.puesto_id = '$unidad ' and ex.colaborador_id = $profesional AND ex.servicio_id = '$servicio'";
}else{
...
$objPHPExcel->getProperties()->setTitle("Reporte Extemporaneos"); //titulo
...
$objPHPExcel->getActiveSheet()->setTitle("Reporte Extemporaneos"); //establecer titulo de hoja
...
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Registro de usuarios extemporaneos $servicio_name");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:N$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:N$fila");
...
$objPHPExcel->getActiveSheet()->SetCellValue("J$fila", 'Puesto');
...
$objPHPExcel->getActiveSheet()->SetCellValue("M$fila", 'Observaciones');
...
if($result->num_rows>0){
	while($registro2 = $result->fetch_assoc()){
	   ...
	   $objPHPExcel->getActiveSheet()->SetCellValue("J$fila", $registro2['puesto']);
	   ...
	   $objPHPExcel->getActiveSheet()->SetCellValue("M$fila", $registro2['observaciones']);
...
// nombre del archivo
header('Content-Disposition: attachment; filename="Reporte de Usuarios Extemporaneos '.$servicio_name.' '.$mes.'_'.$año.'.xls"');
