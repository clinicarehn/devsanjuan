...
if($unidad == ""){
	$where = "WHERE so.fecha BETWEEN '$desde' AND '$hasta' AND so.servicio_id = '$servicio'";
}else{
	...
	$where = "WHERE so.fecha BETWEEN '$desde' AND '$hasta' AND so.servicio_id = '$servicio' AND c.colaborador_id = '$colaborador_usuario'";
}

//EJECUTAMOS LA CONSULTA DE BUSQUEDA
//REGISTROS

$registro = "SELECT CONCAT(c.nombre,' ',c.apellido) AS 'colaborador', (CASE WHEN so.tipo_cita = 'N' THEN 'Nuevo' ELSE 'Subsiguiente' END) AS 'Paciente',
COUNT(CASE WHEN DAY(so.fecha) = 1 THEN so.tipo_cita END) AS '1',  
COUNT(CASE WHEN DAY(so.fecha) = 2 THEN so.tipo_cita END) AS '2',
COUNT(CASE WHEN DAY(so.fecha) = 3 THEN so.tipo_cita END) AS '3',
COUNT(CASE WHEN DAY(so.fecha) = 4 THEN so.tipo_cita END) AS '4',
COUNT(CASE WHEN DAY(so.fecha) = 5 THEN so.tipo_cita END) AS '5',
COUNT(CASE WHEN DAY(so.fecha) = 6 THEN so.tipo_cita END) AS '6',
COUNT(CASE WHEN DAY(so.fecha) = 7 THEN so.tipo_cita END) AS '7',
COUNT(CASE WHEN DAY(so.fecha) = 8 THEN so.tipo_cita END) AS '8',
COUNT(CASE WHEN DAY(so.fecha) = 9 THEN so.tipo_cita END) AS '9',
COUNT(CASE WHEN DAY(so.fecha) = 10 THEN so.tipo_cita END) AS '10',
COUNT(CASE WHEN DAY(so.fecha) = 11 THEN so.tipo_cita END) AS '11',  
COUNT(CASE WHEN DAY(so.fecha) = 12 THEN so.tipo_cita END) AS '12',
COUNT(CASE WHEN DAY(so.fecha) = 13 THEN so.tipo_cita END) AS '13',
COUNT(CASE WHEN DAY(so.fecha) = 14 THEN so.tipo_cita END) AS '14',
COUNT(CASE WHEN DAY(so.fecha) = 15 THEN so.tipo_cita END) AS '15',
COUNT(CASE WHEN DAY(so.fecha) = 16 THEN so.tipo_cita END) AS '16',
COUNT(CASE WHEN DAY(so.fecha) = 17 THEN so.tipo_cita END) AS '17',
COUNT(CASE WHEN DAY(so.fecha) = 18 THEN so.tipo_cita END) AS '18',
COUNT(CASE WHEN DAY(so.fecha) = 19 THEN so.tipo_cita END) AS '19',
COUNT(CASE WHEN DAY(so.fecha) = 20 THEN so.tipo_cita END) AS '20',
COUNT(CASE WHEN DAY(so.fecha) = 21 THEN so.tipo_cita END) AS '21',  
COUNT(CASE WHEN DAY(so.fecha) = 22 THEN so.tipo_cita END) AS '22',
COUNT(CASE WHEN DAY(so.fecha) = 23 THEN so.tipo_cita END) AS '23',
COUNT(CASE WHEN DAY(so.fecha) = 24 THEN so.tipo_cita END) AS '24',
COUNT(CASE WHEN DAY(so.fecha) = 25 THEN so.tipo_cita END) AS '25',
COUNT(CASE WHEN DAY(so.fecha) = 26 THEN so.tipo_cita END) AS '26',
COUNT(CASE WHEN DAY(so.fecha) = 27 THEN so.tipo_cita END) AS '27',
COUNT(CASE WHEN DAY(so.fecha) = 28 THEN so.tipo_cita END) AS '28',
COUNT(CASE WHEN DAY(so.fecha) = 29 THEN so.tipo_cita END) AS '29',
COUNT(CASE WHEN DAY(so.fecha) = 30 THEN so.tipo_cita END) AS '30',
COUNT(CASE WHEN DAY(so.fecha) = 31 THEN so.tipo_cita END) AS '31',
COUNT(so.tipo_cita) AS 'Total'
FROM sobrecupo AS so
INNER JOIN colaboradores AS c
ON so.colaborador_id = c.colaborador_id
".$where."
GROUP BY c.colaborador_id, so.tipo_cita";
...
$fila=2;
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Reporte de Sobre Cupo. ".$servicio_name.' '.$unidad_name);
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:AH$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:AH$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="REPORTE DIARIO DE ATENCIONES SOBRE CUPO'.strtoupper($servicio_name).' '.strtoupper($unidad_name).' '.strtoupper($mes).'_'.$año.'.xls"');
