...
$registro = "SELECT p.pacientes_id AS 'pacientes_id', a.ausencia_id AS 'ausencia_id', (CASE WHEN a.paciente = 'n' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN a.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente', DATE_FORMAT(a.fecha, '%d/%m/%Y') AS 'fecha', CONCAT(p.apellido,' ',p.nombre) AS 'nombre', p.identidad AS 'identidad', a.expediente AS 'expediente', a.comentario AS 'comentario', s.nombre AS 'servicio', CONCAT(c.nombre,' ',c.apellido) AS 'medico', CONCAT(c1.nombre,' ',c1.apellido) AS 'usuario', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', CONCAT(c1.apellido,' ',c1.nombre) AS 'usuario', p.telefono AS 'telefono', s.servicio_id AS 'servicio_id', c.colaborador_id AS 'colaborador_id', a.fecha AS 'fecha_ausencia', d.nombre AS 'departamento', m.nombre AS 'municipio', a.paciente AS 'paciente'
      FROM ausencias AS a
