...
if($unidad == ""){
	$where = "WHERE cast(a.fecha_cita as date) = '$fecha' AND a.servicio_id = '$servicio' AND a.status = '$atencion'";
}else if($medico_general == "" && $unidad != ""){
	$where = "WHERE cast(a.fecha_cita as date) = '$fecha' AND a.servicio_id = '$servicio' AND pc.puesto_id = '$unidad' AND a.status = '$atencion'";
}else{
	...
	$where = "WHERE cast(a.fecha_cita as date) = '$fecha' AND c.colaborador_id = '$medico_general' AND a.servicio_id = '$servicio' AND a.status = '$atencion'";
}
//REGISTROS
...
$fila=4;
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "USUARIOS ".strtoupper($atencion_nombre)." ".strtoupper($mes)." $dia, $año");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:P$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:P$fila");
