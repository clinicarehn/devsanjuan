...
$query_paciente = "SELECT CONCAT(nombre,' ',apellido) AS 'usuario', identidad
   FROM pacientes
   WHERE pacientes_id = '$pacientes_id'";
$result_pacientes = $mysqli->query($query_paciente) or die($mysqli->error);
$registro_pacientes = $result_pacientes->fetch_assoc(); 	   

$nombre_paciente = "";
$identidad_paciente = "";

if($result_ausencias->num_rows>0){
	$nombre_paciente = $registro_pacientes['usuario'];
	$identidad_paciente = $registro_pacientes['identidad'];	
}
...
$tabla = $tabla.'<table class="table table-striped table-condensed table-hover">
	...
	$tabla = $tabla.'<tr>
	    <b>Usuario:</b> '.$nombre_paciente.'   <b>Identidad:</b> '.$identidad_paciente.'
        <td>'.$fecha_cita_.'</td>	
