...
if($servicio != "" && $unidad == "" && $profesional == ""){
  ...
  $where = "WHERE CAST(a.fecha_cita AS DATE) BETWEEN '$desde' AND '$hasta' AND a.servicio_id = '$servicio' AND a.preclinica = '0' AND c.puesto_id = '2' AND c.colaborador_id = '$profesional'";
}
	
$registro = "SELECT a.agenda_id AS 'agenda_id', DATE_FORMAT(CAST(a.fecha_cita AS DATE), '%d/%m/%Y') AS 'fecha', p.identidad AS 'identidad', CONCAT(p.apellido,' ',p.nombre) AS 'nombre', a.expediente AS 'expediente', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', s.nombre AS 'servicio', CONCAT(c.nombre,' ',c.apellido) AS 'medico', (CASE WHEN a.paciente = 'N' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN a.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente', a.observacion AS 'observacion', a.comentario AS 'comentario', CONCAT(c1.apellido,' ',c1.nombre) AS 'usuario'
       FROM agenda AS a
...
$objPHPExcel->getActiveSheet()->setTitle("Reporte Preclínica"); //establecer titulo de hoja
...
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Registro de Usuarios Pendientes en Preclínica $servicio_name");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:N$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:N$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="Reporte de Usuarios Pendientes en Preclínica '.$servicio_name.' '.$mes.'_'.$año.'.xls"');
