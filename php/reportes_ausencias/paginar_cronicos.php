...
if($servicio != "" && $unidad == "" && $profesional == "" && $colaborador_usuario == ""){
	$where = "WHERE u.fecha BETWEEN '$desde' AND '$hasta' AND u.servicio_id = '$servicio' AND u.cronico = 1 AND (CONCAT(p.nombre,' ',p.apellido) LIKE '%$dato%' OR p.apellido LIKE '$dato%' OR p.identidad LIKE '$dato%')";		
}else if ($servicio != "" && $unidad != "" && $profesional == "" && $colaborador_usuario == ""){
   $where = "WHERE u.fecha BETWEEN '$desde' AND '$hasta' AND u.servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND u.cronico = 1 AND (p.expediente LIKE '%$dato%' OR CONCAT(p.nombre,' ',p.apellido) LIKE '%$dato%' OR p.apellido LIKE '$dato%' OR p.identidad LIKE '$dato%')";				
}else if ($servicio != "" && $unidad != "" && $profesional != "" && $colaborador_usuario == ""){	 	
   $where = "WHERE u.fecha BETWEEN '$desde' AND '$hasta' AND u.servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND u.colaborador_id = '$profesional' AND u.cronico = 1 AND (p.expediente LIKE '%$dato%' OR CONCAT(p.nombre,' ',p.apellido) LIKE '%$dato%' OR p.apellido LIKE '$dato%' OR p.identidad LIKE '$dato%')";
}else{
   $where = "WHERE u.fecha BETWEEN '$desde' AND '$hasta' AND u.servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND a.usuario = '$colaborador_usuario' AND u.cronico = 1 AND (p.expediente LIKE '%$dato%' OR CONCAT(p.nombre,' ',p.apellido) LIKE '%$dato%' OR p.apellido LIKE '$dato%' OR p.identidad LIKE '$dato%')";		
}
	  
$query = "SELECT p.pacientes_id AS 'pacientes_id', u.user_cronico_id AS 'user_cronico_id', 
...
$registro = "SELECT p.pacientes_id AS 'pacientes_id', u.user_cronico_id AS 'user_cronico_id', 
  ...
  ON u.servicio_id = s.servicio_id	 	  
  ".$where."
  ORDER BY u.fecha ASC LIMIT $limit, $nroLotes";
...
$tabla = $tabla.'<table class="table table-striped table-condensed table-hover">
			...
			<th width="2%">N</th>
			<th width="2%">S</th>				
			<th width="17%">Servicio</th>
			<th width="18%">Profesional</th>						
			</tr>';			
