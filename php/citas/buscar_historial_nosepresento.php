...
$query = "SELECT DISTINCT p.expediente AS 'expediente', p.identidad As 'identidad', CONCAT(p.apellido, ' ', p.nombre) AS 'paciente_nombre', CONCAT(c.nombre,' ',c.apellido) AS 'colaborador', DATE_FORMAT(CAST(a.fecha_cita AS DATE), '%d/%m/%Y') AS 'fecha_cita', DATE_FORMAT(CAST(a.fecha_registro AS DATE), '%d/%m/%Y') AS 'fecha_registro', a.observacion AS 'observacion', a.hora as 'hora', a.comentario AS 'comentario', s.nombre AS 'servicio'
         FROM agenda AS a
         ...
         WHERE a.status = 2 AND (p.expediente LIKE '$dato%' OR CONCAT(p.nombre,' ',p.apellido) like '$dato%' OR CONCAT(p.apellido,' ',p.nombre) like '$dato%' OR p.identidad like '$dato%' OR CONCAT(c.nombre,' ',c.apellido) like '$dato%')
...
$registro = "SELECT DISTINCT p.expediente AS 'expediente', p.identidad As 'identidad', CONCAT(p.apellido, ' ', p.nombre) AS 'paciente_nombre', CONCAT(c.nombre,' ',c.apellido) AS 'colaborador', DATE_FORMAT(CAST(a.fecha_cita AS DATE), '%d/%m/%Y') AS 'fecha_cita', DATE_FORMAT(CAST(a.fecha_registro AS DATE), '%d/%m/%Y') AS 'fecha_registro', a.observacion AS 'observacion', a.hora as 'hora', a.comentario AS 'comentario', s.nombre AS 'servicio'
         FROM agenda AS a
         INNER JOIN pacientes AS p
         ON a.pacientes_id = p.pacientes_id
         INNER JOIN colaboradores AS c
