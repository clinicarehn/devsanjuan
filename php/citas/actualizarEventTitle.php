...
$end = $_POST['fecha_citaeditend'];	
$fecha_sistema = date("Y-m-d H:i:s");
$fecha_registro = date("Y-m-d H:i:s");
$usuario = $_SESSION['colaborador_id'];
$comentario = cleanStringStrtolower($_POST['coment']);
$comentario1 = cleanStringStrtolower($_POST['coment1']);
...
$consultar_medico = "SELECT agenda_id 
	 FROM agenda 
	 WHERE colaborador_id = '$colaborador_id' AND fecha_cita = '$start' AND fecha_cita_end = '$end' AND status = 0 AND servicio_id = '$servicio'";
...
$query_ausencia = "SELECT a.agenda_id AS 'agenda_id'
	...
	WHERE a.pacientes_id = '$pacientes_id' AND c.puesto_id = '$puesto_colaborador' AND a.servicio_id = '$servicio' AND a.status = 2 AND CAST(a.fecha_cita AS DATE) = '$fecha_cita_cambio'";
		 ...
		 if ( $consultar_medico1['agenda_id'] == ""){
			if ( $consultar_usuario1['agenda_id'] == ""){
				if(getDisponibilidadDiponibilidadHorarioColaborador($colaborador_id, $start) == 2){
					...
					echo 7;//EL MEDIO TIENE ESTA HORA OCUPADA EN OTRO SERVICIO
				}
			}else{
