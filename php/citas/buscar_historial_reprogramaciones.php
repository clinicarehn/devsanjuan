...
$query = "SELECT DATE_FORMAT(CAST(li.fecha_solicitud AS DATE), '%d/%m/%Y') AS 'fecha_solicitud', p.expediente AS 'expediente', CONCAT(p.apellido,' ',p.nombre) AS 'paciente', 
    ...
    WHERE reprogramo = 'X' AND (p.expediente LIKE '$dato%' OR CONCAT(p.apellido,' ',p.nombre) like '$dato%' OR CONCAT(p.nombre,' ',p.apellido) like '$dato%' OR p.identidad like '$dato%' OR CONCAT(c.nombre,' ',c.apellido) like '$dato%')
...
$registro = "SELECT DATE_FORMAT(CAST(li.fecha_solicitud AS DATE), '%d/%m/%Y') AS 'fecha_solicitud', p.expediente AS 'expediente', CONCAT(p.apellido,' ',p.nombre) AS 'paciente', DATE_FORMAT(li.fecha_cita, '%d/%m/%Y') AS 'fecha_cita', CONCAT(c.nombre,' ',c.apellido) AS 'colaborador', CONCAT(c1.nombre,' ',c1.apellido) AS 'usuario', s.nombre AS 'servicio'
    FROM lista_espera AS li
    INNER JOIN pacientes AS p
    ON li.pacientes_id = p.pacientes_id
    INNER JOIN colaboradores AS c
