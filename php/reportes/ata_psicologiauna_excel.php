...
$registro = "SELECT CONCAT(p.nombre,' ',p.apellido) AS 'Nombre', p.expediente AS 'Expediente', p.identidad AS 'Identidad', a.años AS 'Edad',
    ...
    (CASE WHEN a.paciente = 'S' THEN 'X' ELSE '' END) AS 'Subsguiente',
     ...
     CONCAT('') AS 'Tiempo_estancia',
     pa.patologia_id AS 'Codigo_CIE-10', pa1.patologia_id AS 'Codigo_CIE-101', pa2.patologia_id AS 'Codigo_CIE-102', 
     pa.nombre AS 'Diagnostico_CIE-10', a.referencia_mayor AS 'referencia_mayor', a.respuesta1 AS 'respuesta1', a.respuesta2 AS 'respuesta2'
     FROM ata AS a
...
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "FORMATO 2 Atenciones en Consulta Externa MES: $mes AÑO: $año");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:P$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:G$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="CONSOLIDADO CONSULTA EXTERNA '.strtoupper($unidad_name).' '.$colaborador_name.' '.strtoupper($mes).'_'.$año.'.xls"');
