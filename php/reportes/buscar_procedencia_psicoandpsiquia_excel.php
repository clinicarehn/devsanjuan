...
$registro_departamentos = "SELECT d.nombre as departamento, COUNT(p.departamento_id) as total 
    ...
    WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id = '$servicio'    
...
$registro_cortes = "SELECT m.nombre as municipios, COUNT(p.municipio_id) as total
    ...
    WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id = '$servicio' AND d.departamento_id = 5
...
// nombre del archivo
header('Content-Disposition: attachment; filename="PROCEDENCIA USUARIOS '.strtoupper($servicio_name).' '.strtoupper($mes).'_'.$año.'.xls"');
