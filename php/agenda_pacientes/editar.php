...
$valores = "SELECT a.agenda_id AS agenda_id, a.pacientes_id AS 'pacientes_id', a.expediente AS 'expediente', CONCAT(p.apellido, ' ', p.nombre) AS 'paciente_nombre', a.hora AS 'hora', a.fecha_cita AS 'fecha',
	   CONCAT(c.nombre, ' ', c.apellido) As doctor, p.telefono AS 'telefono', c.colaborador_id AS 'colaborador', a.observacion AS 'observacion', a.comentario as 'comentario', a.status_id AS 'status_id', a.servicio_id AS 'servicio_id'
       FROM agenda AS a 
