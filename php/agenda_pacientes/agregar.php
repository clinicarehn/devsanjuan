...
$color_repro = "#FF5733";//ROJO BRILLANTE
...
$consultarJornada = "SELECT j_colaborador_id, nuevos, subsiguientes, servicio_id 
	...
	WHERE colaborador_id = '$colaborador_id' AND servicio_id = '$servicio_usuario'";
...
$valores_array = getAgendatime($consultarJornadaJornada_id, $servicio_usuario, $consultarJornadaServicio_id, $puesto_id, $consulta_agenda_id, $hora_h, $consulta_nuevos_devuelto, $consultarJornadaNuevos, $consultaJornadaTotal, $consulta_subsiguientes_devuelto);
...
$query_ausencia = "SELECT agenda_id
	FROM agenda
	WHERE pacientes_id = '$pacientes_id' AND colaborador_id = '$colaborador_id' AND servicio_id = '$servicio' AND status = 2 AND CAST(fecha_cita AS DATE) = '$nueva_fecha1'";
$result_ausencias = $mysqli->query($query_ausencia);

if($result_ausencias->num_rows==0){
	if($estado_usuario == 3){
		echo 8; //ESTE USUARIO YA FALLECIO
	}else if($estado_usuario == 2){
		echo 9; //ES UN USUARIO PASIVO DEBE CONVERTIRSE A USUARIO ACTIVO ANTES DE PROCEDER.
	}else if($hora == 'Nula' || $hora == 'NulaN' || $hora == 'NulaS' || $hora == 'NulaP' || $hora == 'NulaSError' || $hora == 'NuevosExcede' || $hora == 'SubsiguienteExcede' || $hora == 'Vacio' || $hora == 'Mayora5anos'){
		echo $hora;
	}else{
	  if($preclinica_usuario == ""){
	   if($result->num_rows > 0 && $consultar_puesto1['puesto_id'] == 1){//Verifica la hora del psicologo y su disponibilidad
		  ...
		  echo 1;
			...
			if(date("D", strtotime ( $nueva_fecha1 ) ) == 'Sat' || date("D", strtotime ( $nueva_fecha1 ) ) == 'Sun'){//EVALUA QUE NO SE ALMACENEN CITAS LOS FINES DE SEMANA
			  echo 6;//NO SE PERMITE AGENDAR UN FIN DE SEMANA
			}else {
				  ...
				  if($preclinica_consulta == ""){
					...
					if($query){
					  ...
					  /*****************************************************/	
					   ...
					   echo 2; //REGISTRO EDITADO Y/O ALMACENADO CORRECTAMENTE
				   }else{
					   echo 3;//ERROR AL PROCESAR ESTA SOLICITUD
				   }		
				}else{
					echo 4;//YA SE HA PRECLINEADO ESTE USUARIO
				}
			  }else{
				  echo 5; //EL USUARIO YA TIENE CITA ESE DIA
			  }
			}else{
		 ...
		 echo 7;//EL USUARIO SE HA PRECLINIADO, NO SE PUEDE REPROGRAMAR SU CITA
	 } 
	}
}else{
	echo 10;//YA EXISTE UNA AUSENCIA PARA ESTE REGISTRO, NO SE PUEDE VOLVER A ALMACENAR, POR FAVOR CANCELE LA AUSENCIA ALMACENADA
}
