		   ...
		   if($result_grupos_nivel->num_rows>0){
			   while($registro_grupos_nivel1 = $result_grupos_nivel->fetch_assoc()){
				   $grupo_niveles_grupo_id  = $registro_grupos_nivel1['niveles_grupo_id'];
					...
					if($servicio !=0){
						$where = "WHERE re.fecha BETWEEN '$desde' AND '$hasta' AND ch.centro_id = '$grupo_niveles_grupo_id' AND re.servicio_id = '$servicio'";						
					}else{
						$where = "WHERE re.fecha BETWEEN '$desde' AND '$hasta' AND ch.centro_id = '$grupo_niveles_grupo_id'";
					}
				   
				   //CONSULTAMOS LA CANTIDAD DE CENTROS SEGUN EL GRUPO
				   $consulta_centros = "SELECT ch.centro_nombre AS 'centro', COUNT(re.referenciar_id) AS 'total', ng.nombre AS 'centro_grupo', nc.nombre AS 'nivel'
						...
						ON re.centro = ng.niveles_grupo_id
						INNER JOIN niveles_centros AS nc
						ON re.nivel = nc.niveles_centros_id
						".$where."
...
if($servicio !=0){
	$where = "WHERE re.fecha BETWEEN '$desde' AND re.servicio_id = '$servicio'";	
}else{
	$where = "WHERE re.fecha BETWEEN '$desde' AND '$hasta'";
}
					
$consulta_total_niveles = "SELECT nc.nombre AS 'nivel', COUNT(re.nivel) AS 'total'
FROM referencia_enviada AS re
...
if($result_niveles_agrupada->num_rows>0){
	while($registro_niveles_agrupada1 = $result_niveles_agrupada->fetch_assoc()){
		...
		if($servicio !=0){
	        $where_consulta = "WHERE re.fecha BETWEEN '$desde' AND re.nivel = '$nivel_id' AND re.servicio_id = '$servicio'";	
        }else{
	        $where_consulta = "WHERE re.fecha BETWEEN '$desde' AND '$hasta' AND re.nivel = '$nivel_id'";
        }
		...
		$objPHPExcel->getActiveSheet()->setSharedStyle($subtitulo, "A$fila:B$fila"); //establecer estilo
		$objPHPExcel->getActiveSheet()->getStyle("A$fila:B$fila")->getFont()->setBold(true); //negrita
		$consultar_total_grupos = "SELECT ng.nombre AS 'grupo', COUNT(re.centro) AS 'total'
			FROM referencia_enviada AS re
...
// nombre del archivo
header('Content-Disposition: attachment; filename="CONTROL DE CENTROS_RE '.strtoupper($mes).'_'.$año.'.xls"');
