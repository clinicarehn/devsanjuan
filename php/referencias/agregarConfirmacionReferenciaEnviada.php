...
//CONFIRMACIONES
if(isset($_POST['confirmo_respuesta_recibida'])){
   if($_POST['confirmo_respuesta_recibida'] == ""){
       $confirmo = 0;	
   }else{
	   $confirmo = $_POST['confirmo_respuesta_recibida'];
   }
}

if(isset($_POST['confirmo_respuesta_recibida1'])){
    if($_POST['confirmo_respuesta_recibida1'] == ""){
        $confirmo1 = 0;	
    }else{
	    $confirmo1 = $_POST['confirmo_respuesta_recibida1'];
    }
}
...
$consultar_existencia = "SELECT confirmacion_re_id 
     FROM confirmacion_re 
...
$consulta_existencia_respuesta_referencia = $consultar_existencia2['confirmacion_re_id'];

//OBTENER CORRELATIVO
$correlativo = "SELECT MAX(confirmacion_re_id) AS max, COUNT(confirmacion_re_id) AS count 
    FROM confirmacion_re";
...
if ($proceso = 'Registro'){
  if($consulta_existencia_respuesta_referencia == ""){
	  ...
	  if($referencia_id != 0 && $servicio_id != 0 && $expediente != 0 && $colaborador_id != 0 && $usuario != 0){
		  $insert = "INSERT INTO confirmacion_re 
          ...
          $estado = "Agregar";
          $observacion = "Se ha agregado una confirmación a la referencia enviada";
          $modulo = "Confirmacion";
          $insert = "INSERT INTO historial 
                VALUES('$historial_numero','$pacientes_id','$expediente','$modulo','$numero','$colaborador_id','$servicio_id','$fecha','$estado','$observacion','$usuario','$fecha_registro')";	 
          $mysqli->query($insert);
          /*****************************************************/			  
	  }else{
		  echo 4;
