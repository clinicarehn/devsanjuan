...
$registro = "SELECT DATE_FORMAT(re.fecha, '%d/%m/%Y') AS 'fecha', CONCAT(c1.apellido,' ',c1.nombre) AS 'usuario', CONCAT(p.apellido,' ',p.nombre) AS 'nombre', p.identidad AS 'identidad', re.expediente AS 'expediente', d.nombre AS 'departamento', m.nombre AS 'municipio', re.clinico AS 'clinico', pa1.patologia_id AS 'patologia1', pa2.patologia_id AS 'patologia2', pa3.patologia_id AS 'patologia3', re.motivo_referencia AS 'motivo', ch.centro_nombre AS 'unidad_enviada', CONCAT(c.apellido,' ',c.nombre) AS 'colaborador', re.respuesta_recibida AS 'resp_recibida', re.edad AS 'edad', p.sexo As 'sexo', ata.paciente AS 'paciente'
