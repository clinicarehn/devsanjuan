...
if($servicio !=0){
	...
	$where = "WHERE rc.servicio_id = '$servicio' AND rc.fecha BETWEEN '$desde' AND '$hasta'";
}else{	
...
$registro = "SELECT rc.expediente AS 'expediente', CONCAT(p.apellido, ' ', p.nombre) AS 'usuario', 
(CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h', 
(CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', 
a.años AS 'edad', mt.nombre AS 'causa', ch.centro_nombre AS 'centro_recibe', rc.fecha AS 'fecha_referencia', rc.fecha AS 'fecha_respuesta', p.identidad AS 'identidad'
FROM referencia_recibida AS rc 
INNER JOIN pacientes AS p 
ON rc.expediente = p.expediente 
INNER JOIN ata AS a 
ON rc.ata_id = a.ata_id 
INNER JOIN motivo_traslado AS mt 
ON rc.motivo_traslado = mt.motivo_traslado_id 
INNER JOIN centros_hospitalarios AS ch 
ON rc.unidad_envia = ch.centros_id 
".$where."
ORDER BY rc.fecha, rc.expediente";
...
$objDrawing->setWidth(135); //anchura
...
$fila=2;
$objPHPExcel->getActiveSheet()->setSharedStyle($bordes, "A4:J4");
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Registro Diario de Referencias Recibidas y Respuestas Enviadas $nombre_servicio");
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:J$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:J$fila");

$fila=3;
$objPHPExcel->getActiveSheet()->setSharedStyle($bordes, "A5:J5");
...
$objPHPExcel->getActiveSheet()->SetCellValue("J$fila", 'Fecha de Respuesta Recibida');
...
if($result->num_rows>0){
	while($registro2 = $result->fetch_assoc()){
	   ...
	   $objPHPExcel->getActiveSheet()->SetCellValue("H$fila", $registro2['centro_recibe']);
	   $objPHPExcel->getActiveSheet()->SetCellValue("I$fila", $registro2['fecha_referencia']);	      
	   $objPHPExcel->getActiveSheet()->SetCellValue("J$fila", $registro2['fecha_referencia']);
...
// nombre del archivo
header('Content-Disposition: attachment; filename="Referencias Recibidas UGD '.$nombre_servicio.'.xls"');
