...
while($dia <=31){
   if($servicio != 0){
	   ...
	   $result_servicios = $mysqli->query($consultar_servicio);
	   $nombre_servicio = $result_servicios['nombre'];
  ...
  $registro = "SELECT COUNT(a.ata_id) AS 'n_atenciones', COUNT(rr.referenciar_id) AS 'n_referencias', COUNT(rr.referenciar_id) as 'n_respuestas'
       ...
       ".$where;  
  $result_servicios = $mysqli->query($registro); 
}
...
if(mysql_num_rows($registro)>0){
	...
	while($mes <= 12){	
        ...
        $registro = mysql_query("SELECT COUNT(a.ata_id) AS 'n_atenciones', COUNT(rr.referenciar_id) AS 'n_referencias', COUNT(rr.referenciar_id) as 'n_respuestas' 
            FROM ata AS a 
            LEFT JOIN referencia_recibida AS rr
            ON a.ata_id = rr.ata_id 
            ".$where
       ...
       //FIN CONSULTAR CANTIDAD DE REFERENCIAS
        ...
        $registro_ata = mysql_query("SELECT COUNT(a.ata_id) AS 'n_atenciones'
            FROM ata AS a
            ".$where_ata
        );
       //FIN CONSULTAR CANTIDAD DE REFERENCIAS	 
       ...
       while($registro2 = mysql_fetch_array($registro)){
		  ...
		  while($registro_ata2 = mysql_fetch_array($registro_ata)){
