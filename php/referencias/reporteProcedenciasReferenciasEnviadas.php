...
if($servicio !=0){
	$where = "WHERE re.fecha BETWEEN '$desde' AND '$hasta' AND re.servicio_id = '$servicio'";
	$where1 = "WHERE re.fecha BETWEEN '$desde' AND '$hasta' AND d.departamento_id = 5 AND re.servicio_id = '$servicio'";
}else{
	$where = "WHERE re.fecha BETWEEN '$desde' AND '$hasta'";
	$where1 = "WHERE re.fecha BETWEEN '$desde' AND '$hasta' AND d.departamento_id = 5";
}	
...
// nombre del archivo
header('Content-Disposition: attachment; filename="PROCEDENCIA USUARIOS RE '.strtoupper($mes).'_'.$año.'.xls"');
