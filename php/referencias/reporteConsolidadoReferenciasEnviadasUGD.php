...
if($servicio != 0){
	...
	$where = "WHERE re.servicio_id = '$servicio' AND re.fecha BETWEEN '$fecha_inicial' AND '$fecha_final'";
}else{
	$nombre_servicio = "";
	$where = "WHERE re.fecha BETWEEN '$fecha_inicial' AND '$fecha_final'";
}
...
$registro = "SELECT COUNT(a.ata_id) AS 'n_atenciones', COUNT(re.referenciar_id) AS 'n_referencias', COUNT(rp_re.respuesta_re_id) as 'n_respuestas'
    FROM ata AS a
    LEFT JOIN referencia_enviada AS re
    ON a.ata_id = re.ata_id
...
$objPHPExcel->getActiveSheet()->SetCellValue("C$fila", 'Referencias Enviadas');
$objPHPExcel->getActiveSheet()->getColumnDimension('C')->setWidth(42); 
$objPHPExcel->getActiveSheet()->mergeCells("C4:D4"); //unir celdas
$objPHPExcel->getActiveSheet()->SetCellValue("E$fila", 'Respuestas Recibidas');
...
if($result->num_rows>0){
	...
	while($mes <= 12){	
        ...
        $registro_atenciones = "SELECT COUNT(a.ata_id) AS 'n_atenciones', COUNT(re.referenciar_id) AS 'n_referencias', COUNT(rp_re.respuesta_re_id) as 'n_respuestas'
            FROM ata AS a
            LEFT JOIN referencia_enviada AS re
            ON a.ata_id = re.ata_id
            LEFT JOIN respuesta_re AS rp_re
            ON re.referenciar_id = rp_re.referencia_id
            ".$where;
	   $result_ata_atenciones = $mysqli->query($registro_atenciones);
       //FIN CONSULTAR CANTIDAD DE REFERENCIAS
        ...
        $registro_ata = "SELECT COUNT(a.ata_id) AS 'n_atenciones'
            FROM ata AS a
            ".$where_ata;
		$result_ata = $mysqli->query($registro_ata);
       //FIN CONSULTAR CANTIDAD DE REFERENCIAS	 

       //INICIO CONSULTAR RESPUESTAS RECIBIDAS
	   $registro_respuestas = "SELECT COUNT(respuesta_re_id) as 'n_respuestas'
	       FROM respuesta_re
		   WHERE fecha BETWEEN '$fecha_inicial' AND '$fecha_final'";
	    $result_respuestas = $mysqli->query($registro_respuestas);
	   
       //INICIO CONSULTAR RESPUESTAS RECIBIDAS	     	   	   	   
   
       //CONSULTAMOS LA CANTIDAD DE REGISTROS PARA PODER LLENAR LOS MESES EN LA CASILLA MES DEL EXCEL
       while($registro2 = $result_ata_atenciones->fetch_assoc()){
		  $fila+=1;
		  ...
		  while($registro_ata2 = $result_ata->fetch_assoc()){
		  ...
		  //CONSULTAMOS LA CANTIDAD DE RESUESTAS RECIBIDAS
		  while($registro_respuestas2 = $result_respuestas->fetch_assoc()){
			  $n_respuestas = $registro_respuestas2['n_respuestas'];
			  //LLENAMOS EL EXCEL CON EL NUMERO DE ATENCIONES
...
// nombre del archivo
header('Content-Disposition: attachment; filename="Consolidado de Referencias Enviadas UGD '.$nombre_servicio.'.xls"');
...
$result_ata_atenciones->free();//LIMPIAR RESULTADO
$result_ata->free();//LIMPIAR RESULTADO
$result_respuestas->free();//LIMPIAR RESULTADO
