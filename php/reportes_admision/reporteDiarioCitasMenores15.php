...
if($unidad == ""){
	$where = "WHERE CAST(lis.fecha_solicitud AS DATE) BETWEEN '$desde ' AND '$hasta' AND lis.servicio = '$servicio' AND lis.prioridad = 'P' AND lis.reprogramo = ''";
}else{
	...
	$where = "WHERE CAST(lis.fecha_solicitud AS DATE) BETWEEN '$desde ' AND '$hasta' AND lis.servicio = '$servicio' AND lis.prioridad = 'P' AND lis.reprogramo = '' AND c.puesto_id = '$unidad'";
}
...
$fila=2;
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "Reporte De Citas Menores de 15 Días ".$servicio_name.' '.$unidad_name);
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:AG$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:AG$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="REPORTE DE CITAS MENORES A 15 DIAS'.strtoupper($servicio_name).' '.strtoupper($unidad_name).' '.strtoupper($mes).'_'.$año.'.xls"');
