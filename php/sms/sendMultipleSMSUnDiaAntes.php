...
if($servicio != "" && $unidad == "" && $medico == ""){
	...
	$where1 = "WHERE fecha = '$fecha' AND servicio_id = '$servicio' AND dias = 1";
    ...
    $where1 = "WHERE fecha = '$fecha' AND servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND dias = 1";	
}else if($servicio != "" && $unidad != "" && $medico != ""){
	...
	$where1 = "WHERE fecha = '$fecha' AND servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND c.colaborador_id = '$medico' AND dias = 1";
}
    ...
    if(!$sock = @fsockopen('www.google.com', 80)){ 
       ...
       while($registro2 = $result_agenda_cosnsulta->fetch_assoc()){ 
	    ...
	    if($valor == 9 || $valor == 8 || $valor == 7 || $valor == 3){//VERIFICAMOS QUE LOS NUMEROS SEAN CELULARES
	        ...
	        $to = $area."".$telefono;  
	        $mensaje = "Estimado(a) $nombre_usuario $nombre_apellido, le recordamos, su cita es el $dia_nombre $dia de $mes, $año. Favor estar 15 minutos antes. Su cita es presencial";
