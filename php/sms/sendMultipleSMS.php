...
if($servicio != "" && $unidad == "" && $medico == ""){
	...
	$where1 = "WHERE fecha = '$fecha' AND servicio_id = '$servicio' AND dias = 5";
}else if($servicio != "" && $unidad != "" && $medico == ""){
	$where  = "WHERE a.servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND CAST(a.fecha_cita AS DATE) = '$fecha' AND a.status = 0 AND LENGTH(p.telefono) = 8 AND p.telefono NOT LIKE '2%'";	
	$where1 = "WHERE fecha = '$fecha' AND servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND dias = 5";
}else if($servicio != "" && $unidad != "" && $medico != ""){
	$where  = "WHERE a.servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND c.colaborador_id = '$medico' AND CAST(a.fecha_cita AS DATE) = '$fecha' AND a.status = 0 AND LENGTH(p.telefono) = 8 AND p.telefono NOT LIKE '2%'";	
	$where1 = "WHERE fecha = '$fecha' AND servicio_id = '$servicio' AND c.puesto_id = '$unidad' AND c.colaborador_id = '$medico' AND dias = 5";
}
    ...
    if(!$sock = @fsockopen('www.google.com', 80)){ 
       ...
       while($registro2 = $result_agenda_cosnsulta->fetch_assoc()){ 
	    ...
	    if($valor == 9 || $valor == 8 || $valor == 7 || $valor == 3){//VERIFICAMOS QUE LOS NUMEROS SEAN CELULARES
	        ...
	        $to = $area."".$telefono; 		
	        $mensaje = str_replace("@nombre_usuario", $nombre_usuario, $text_);
			$mensaje = str_replace("@nombre_apellido", $nombre_apellido, $mensaje);
			$mensaje = str_replace("@dia_nombre", $dia_nombre, $mensaje);
			$mensaje = str_replace("@dia", $dia, $mensaje);
			$mensaje = str_replace("@mes", $mes, $mensaje);
			$mensaje = str_replace("@año", $año, $mensaje);
