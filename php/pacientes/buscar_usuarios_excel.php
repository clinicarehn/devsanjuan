...
if ($dato == ""){
      ...
      $query = "SELECT DISTINCT p.pacientes_id AS pacientes_id, p.expediente AS expediente, p.nombre AS nombre, p.apellido AS apellido, 
             ...
             LEFT JOIN municipios AS m
             ON p.municipio_id = m.municipio_id  
			 WHERE p.status = '$estado' AND p.expediente LIKE '$dato%' OR CONCAT(p.nombre,' ',p.apellido) like '$dato%'  OR p.identidad like '$dato%' OR p.localidad LIKE '%$dato%'
             ORDER BY expediente LIMIT 30";		
}
...
$objPHPExcel->getActiveSheet()->SetCellValue("D$fila", 'Apellido');
...
$objPHPExcel->getActiveSheet()->SetCellValue("H$fila", 'Teléfonos1');
$objPHPExcel->getActiveSheet()->getColumnDimension('H')->setWidth(13); 
$objPHPExcel->getActiveSheet()->SetCellValue("I$fila", 'Teléfonos2');
$objPHPExcel->getActiveSheet()->getColumnDimension('I')->setWidth(13);
$objPHPExcel->getActiveSheet()->SetCellValue("J$fila", 'Teléfonos3');
$objPHPExcel->getActiveSheet()->getColumnDimension('J')->setWidth(13);
$objPHPExcel->getActiveSheet()->SetCellValue("K$fila", 'Teléfonos4');
...
if($result->num_rows>0){
	while($registro1 = $result->fetch_assoc()){
	  if($registro1['telefono1'] == 0 || $registro1['telefono1'] == ""){
		  $telefono1 = "";
	  }else{
		  $telefono1 = $registro1['telefono1'];  
	  }
	  
	  if($registro1['telefonoresp'] == 0 || $registro1['telefonoresp'] == ""){
		  $telefonoresp = "";
	  }else{
		  $telefonoresp = $registro1['telefonoresp'];  
	  }	  
	  
	  if($registro1['telefonoresp1'] == 0 || $registro1['telefonoresp1'] == ""){
		  $telefonoresp1 = "";
	  }else{
		  $telefonoresp1 = $registro1['telefonoresp1'];  
	  }
	  if ($registro1['expediente'] == 0){
		  ...
		  $expediente = $registro1['expediente'];
           ...
           $objPHPExcel->getActiveSheet()->SetCellValue("B$fila", $expediente);
           $objPHPExcel->getActiveSheet()->SetCellValue("C$fila", $registro1['nombre']);
           $objPHPExcel->getActiveSheet()->SetCellValue("D$fila", $registro1['apellido']);
		   $objPHPExcel->getActiveSheet()->setCellValueExplicit("E$fila", $registro1['identidad'], PHPExcel_Cell_DataType::TYPE_STRING);
           $objPHPExcel->getActiveSheet()->SetCellValue("F$fila", $registro1['sexo']);
           $objPHPExcel->getActiveSheet()->SetCellValue("G$fila", $registro1['fecha_nacimiento']);
           ...
           $objPHPExcel->getActiveSheet()->SetCellValue("J$fila", $telefonoresp);		   
           $objPHPExcel->getActiveSheet()->SetCellValue("K$fila", $telefonoresp1);
           $objPHPExcel->getActiveSheet()->SetCellValue("L$fila", $registro1['departamento']);
           $objPHPExcel->getActiveSheet()->SetCellValue("M$fila", $registro1['municipio']); 
           $objPHPExcel->getActiveSheet()->SetCellValue("N$fila", $registro1['localidad']); 		   
...
// nombre del archivo
header('Content-Disposition: attachment; filename="REPORTE DE USUARIOS '.strtoupper($mes).' '.$dia.', '.$año.'.xls"');
