...
if($servicio != "" && $unidad == ""){
	$where = "WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id IN(".$servicio.")";
	$where1 = "WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id IN(".$servicio.") AND d.departamento_id = 5";
}else{
	...
	$where = "WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id IN(".$servicio.") AND c.puesto_id = '$unidad'";
	$where1 = "WHERE a.fecha BETWEEN '$desde' AND '$hasta' AND a.servicio_id IN(".$servicio.") AND d.departamento_id = 5 AND c.puesto_id = '$unidad'";
}
...
$fila=5;
$objPHPExcel->getActiveSheet()->SetCellValue("A$fila", "PROCEDENCIA DE USUARIOS ".strtoupper($unidad_name));
$objPHPExcel->getActiveSheet()->mergeCells("A$fila:D$fila"); //unir celdas
$objPHPExcel->getActiveSheet()->setSharedStyle($titulo, "A$fila:D$fila");
...
// nombre del archivo
header('Content-Disposition: attachment; filename="PROCEDENCIA USUARIOS '.strtoupper($servicio_name).' '.strtoupper($unidad_name).' '.strtoupper($mes).'_'.$año.'.xls"');
