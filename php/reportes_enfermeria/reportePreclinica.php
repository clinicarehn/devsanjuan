...
$registro = "SELECT DATE_FORMAT(pre.fecha, '%d/%m/%Y') AS 'fecha', CONCAT(p.apellido,' ',p.nombre) AS 'nombre', pre.expediente As 'expediente', p.identidad AS 'identidad', pre.edad AS 'edad', (CASE WHEN p.sexo = 'H' THEN 'X' ELSE '' END) AS 'h',
      (CASE WHEN p.sexo = 'M' THEN 'X' ELSE '' END) AS 'm', (CASE WHEN pre.paciente = 'n' THEN 'X' ELSE '' END) AS 'nuevo', (CASE WHEN pre.paciente = 'S' THEN 'X' ELSE '' END) AS 'subsiguiente', pre.pa AS 'pa', pre.fr As 'fr', pre.fc As 'fc', pre.peso AS 'peso', pre.t As 'temperatura', pre.talla AS 'talla', CONCAT(c.nombre,' ',c.apellido) AS 'medico', CONCAT(c1.nombre,' ',c1.apellido) AS 'usuario', pregluco.glucometria AS 'glucometria'
       ...
       ".$where."
       ORDER BY pre.fecha, p.expediente";
...
if($result->num_rows>0){
	while($registro2 = $result->fetch_assoc()){
	   ...
	   $objPHPExcel->getActiveSheet()->SetCellValue("Q$fila", $registro2['glucometria']);	   
	   $objPHPExcel->getActiveSheet()->SetCellValue("R$fila", $registro2['medico']);
	   $objPHPExcel->getActiveSheet()->SetCellValue("S$fila", $registro2['usuario']);
