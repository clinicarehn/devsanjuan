	...
	var comprobar = $('#file-0a').val().length;
	  if(comprobar>0){
		...
		$.ajax({
		  ...
		  beforeSend : function (){
			  $('#respuesta').html('<img src="../img/gif-load.gif" width="5%" height="5%">');	
		  },
		...
		success: function(resp){
		  if(resp == 'OK'){
				swal({
					title: "Success", 
					text: "Importación de CSV exitosa",
					type: "success",
					timer: 3000,
...
function bajar(){
	...
	if (selector == "ata")
		   window.location = "../files/formato_sistema_atas.xlsx";
		else if (selector == "pacientes")
    	   window.location = "../files/formato_sistema_pacientes.xlsx"
		else if (selector == "agenda")
		   window.location = "../files/formato_sistema_agenda.xlsx"
		else if (selector == "patologias")
		   window.location = "../files/formato_sistema_patologias.xlsx"	
		else if (selector == "activos")
		   window.location = "../files/formato_activo.xlsx"	
		else if (selector == "pasivos")
		   window.location = "../files/formato_pasivo.xlsx"		
        else if (selector == "centros_hospitalarios")
		   window.location = "../files/centros_hospitalario_subir.xlsx"		   
}
